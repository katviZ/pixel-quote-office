...
(function (root, factory) {
  ...
  function computeQuote(params, db) {
    ...
    // ---- pricing (shared, with OFFICE overrides layered on top) ----
    // Each override is opt-in — passing null/undefined keeps catalog behaviour.
    // Passing 0 IS honoured (means "free" — deliberate override, not fallback).
    // Every overridden line gets `overridden: true` so the UI can badge it.
    const pr = priceForPitch(product, pitch);
    const hasOverride = (v)=> v != null && v !== "" && !Number.isNaN(+v);
    // LED rate
    const rateFinal = hasOverride(params.rateOverride) ? +params.rateOverride : pr.rate;
    const screenCost = round(rateFinal * areaSqft);
    const screenGst = round((screenCost * pr.gst) / 100);
    const rateOverridden = hasOverride(params.rateOverride) && +params.rateOverride !== pr.rate;
    // Controller price
    const ctrlCatalog = controllerPrice(db, ports, micro);
    const ctrlCost = hasOverride(params.ctrlPriceOverride) ? +params.ctrlPriceOverride : ctrlCatalog;
    const ctrlGstPct = db.meta.controllerGst;
    const ctrlGst = round((ctrlCost * ctrlGstPct) / 100);
    const ctrlOverridden = hasOverride(params.ctrlPriceOverride) && +params.ctrlPriceOverride !== ctrlCatalog;
    // Installation rate
    const iRateCatalog = installRate(db, areaSqft);
    const iRate = hasOverride(params.installRateOverride) ? +params.installRateOverride : iRateCatalog;
    ...
    const installOverridden = hasOverride(params.installRateOverride) && +params.installRateOverride !== iRateCatalog;

    const lines = [
      { item: "LED Display", rate: rateFinal, rateUnit: "/sqft", qty: areaSqft.toFixed(1) + " sqft", amount: screenCost, gstPct: pr.gst, gst: screenGst, overridden: rateOverridden, catalogRate: pr.rate, key: "ledRate" },
      { item: "Video Controller", rate: ctrlCost, rateUnit: " /system", qty: ports + " port(s)", amount: ctrlCost, gstPct: ctrlGstPct, gst: ctrlGst, overridden: ctrlOverridden, catalogRate: ctrlCatalog, key: "ctrlPrice" },
      { item: "Installation", rate: iRate, rateUnit: "/sqft", qty: Math.ceil(areaSqft) + " sqft", amount: installCost, gstPct: installGstPct, gst: installGst, overridden: installOverridden, catalogRate: iRateCatalog, key: "installRate" },
    ];
    ...
    // ---- OFFICE custom lines — appended to the standard line list ----
    // Each entry: { item, rate, qty, gstPct } — engine computes amount + gst.
    // Malformed entries silently dropped (defensive against localStorage rot).
    if (Array.isArray(params.customLines)) {
      for (const cl of params.customLines) {
        if (!cl || !cl.item) continue;
        const clRate = +cl.rate;
        const clQty = +cl.qty;
        const clGstPct = +cl.gstPct;
        if (!Number.isFinite(clRate) || !Number.isFinite(clQty)) continue;
        const clAmount = round(clRate * clQty);
        const clGst = Number.isFinite(clGstPct) ? round((clAmount * clGstPct) / 100) : 0;
        lines.push({
          item: String(cl.item),
          rate: clRate,
          rateUnit: cl.rateUnit || " /unit",
          qty: String(cl.qtyLabel || clQty),
          amount: clAmount,
          gstPct: Number.isFinite(clGstPct) ? clGstPct : 0,
          gst: clGst,
          overridden: true,
          custom: true,
          key: "custom:" + (cl.id || cl.item),
        });
      }
    }
  ...
  function findFits(params, db) {
    ...
    product.cabinets.forEach((cab, cabIndex) => {
      ...
      orientations.forEach((orientation) => {
        ...
        // Same 3mm tolerance as computeQuote — kills round-trip drift so
        // clicking "Optimal Fit" doesn't produce a bigger fit than what
        // the card previously advertised (the multi-click multiplication bug).
        const FF_TOL_MM = 3;
        const ncWup = Math.max(1, Math.ceil((reqW - FF_TOL_MM) / cw));
        const ncHup = Math.max(1, Math.ceil((reqH - FF_TOL_MM) / ch));
        ...
        // Mirror the ceil tolerance: adding 3mm BEFORE floor stops the
        // round-trip drift from dropping "Nearest Fit" one cabinet lower
        // than the previous click's built dim (the reverse multi-click bug).
        const ncWdn = Math.floor((reqW + FF_TOL_MM) / cw);
        const ncHdn = Math.floor((reqH + FF_TOL_MM) / ch);
        if (ncWdn >= 1 && ncHdn >= 1) {
